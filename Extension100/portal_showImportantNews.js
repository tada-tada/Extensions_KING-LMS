async function showImportantNews() {
	...
	if(!searchImportantWord) {
		searchImportantWord = ["行事","文化教養","体育行事","ハイキング","履修","単位","卒業","奨学","記念日","健康診断","学業","学期","提出","手続き","定期","実施","臨時便","シャトルバス","校舎"];
	}
	...
	const interval = setInterval(() => {
		...
		newsList.setAttribute('class', "general footable news-list prj_common");
		...
		if(document.querySelector("#impNews_details").getElementsByClassName("newArrivalBold").length !== 0) {
			let unread = document.createElement("span");
			unread.textContent = "（未読のお知らせがあります）"

			document.getElementById("impNews_details").getElementsByClassName("subTitle")[0].textContent += unread.textContent;
		}
	...
	const interval_news = setInterval(() => {
		...
		commonNews_details.setAttribute("id", "commonNews_details");
		...
		if(document.querySelector("#commonNews_details").getElementsByClassName("newArrivalBold").length !== 0) {
			let unread = document.createElement("span");
			unread.textContent = "（未読のお知らせがあります）";
			document.getElementById("commonNews_details").getElementsByClassName("subTitle")[0].textContent += unread.textContent;
		}		

		if(document.getElementsByClassName("prj_common")[0].children[1].children.length === 0) {
			document.getElementById("commonNews_details").children[2].setAttribute("style","display:none;");
			document.getElementsByClassName("noDataHomeNews")[0].setAttribute("style","");
		}
...
function removeImportantNewsTag(ipNews_removeNumber) {
	let remove_newsList = document.getElementsByClassName("prj_common")[0].children[1].children;
